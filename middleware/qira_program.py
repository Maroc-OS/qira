...
####################################################
# Those following, needs too be moved into arch.py #
####################################################
...
def which(prog):
    try:
        cmd = ["which", prog]
        pro = Popen(cmd, stdout=PIPE)
        res = pro.stdout.readlines()
        ...
        if os.path.isfile(prog):
            ...
            raise Exception("Error '{0}' occured.".format(err.message))
...
class Program(object):
    ...
    def identify_program(self):
        ...
        # Windows binaries
            ...
            if wh == 0x14c:
                ...
                raise Exception(
                    "windows binary with machine " +
                    hex(wh) +
                    " not supported")

        # MACHO FAT binaries
        elif progdat[0x0:0x04] in (MACHO_FAT_MAGIC,
                                   MACHO_FAT_CIGAM,
                                   MACHO_P200_FAT_MAGIC,
                                   MACHO_P200_FAT_CIGAM):
            print "**** Mach-O FAT (Universal) binary detected"

            if (progdat[0x04:0x05] == CPU_TYPE_ARM) and (
                    progdat[0x08:0x09] in CPU_SUBTYPE_ARM):
                print "**** Mach-O ARM architecture detected"
                self.macharch = "arm"
            elif ((progdat[0x08:0x0c] == CPU_TYPE_ARM64) or
                  (progdat[0x1c:0x20] == CPU_TYPE_ARM64) or
                  (progdat[0x30:0x34] == CPU_TYPE_ARM64)):
            ...
            if progdat[0x0:0x04] in (
                    MACHO_P200_FAT_MAGIC,
                    MACHO_P200_FAT_CIGAM):
                raise NotImplementedError(
                    "Pack200 compressed files are not supported yet")
            elif progdat[0x0:0x04] in (
                    MACHO_FAT_MAGIC,
                    MACHO_FAT_CIGAM):
            ...
            if self.macharch == "arm" or self.macharch == "aarch64":
                raise NotImplementedError(
                    "ARM/Aarch64 Support is not implemented")
            if not os.path.isfile(self.pintool):
                print "Running a Mach-O FAT (Universal) binary requires PIN support.\n"
                print "See tracers/pin_build.sh\n"
                exit()
            self.runnable = True

            # Defaulting to NotImplimented Until we got full Support
            raise NotImplementedError(
                "Mach-O FAT (Universal) binary not supported")

        # MACHO binaries
        elif progdat[0x0:0x04] in (MACHO_MAGIC_64,
                                   MACHO_CIGAM_64,
                                   MACHO_MAGIC,
                                   MACHO_CIGAM):
            ...
            if ((progdat[0x04:0x05] == CPU_TYPE_ARM) and
                ...
                print "**** Mach-O ARM architecture detected"
                self.macharch = "arm"
            elif ((progdat[0x04:0x05] == CPU_TYPE_ARM) and
                  (progdat[0x08:0x09] in CPU_SUBTYPE_ARM64)):
                print "**** Mach-O Aarch64 architecture detected"
                self.macharch = "aarch64"
            else:
                self.macharch = ""
    ...
    def read_asm_file(self):
        ...
        for d in dat.split("\n"):
            ...
            try:
                if self.fb == 0x28:
                    # thumb bit in front
                    addr = fhex(d.split(" ")[0][1:].strip(":"))
                else:
                    addr = fhex(d.split(" ")[0].strip(":"))
            except Exception as err:
                continue
            if self.fb == 0x28:
                thumb_flag = d[0]
                if thumb_flag == 't':
                    ...
                    # print "*** Invalid thumb flag at beginning of
                    # instruction"
                    pass
                inst = d[d.rfind("  ") + 2:]
    ...
    def get_maxclnum(self):
        ...
        for t in self.traces:
            ret[t] = [
                self.traces[t].db.get_minclnum(),
                self.traces[t].db.get_maxclnum()]
        return ret
    ...
    def add_trace(self, fn, i):
        self.traces[i] = Trace(
            fn, i, self, self.tregs[1], len(
                self.tregs[0]), self.tregs[2])
...
class Trace(object):
    ...
    def fetch_raw_memory(self, clnum, address, ln):
        return ''.join(
            map(chr, self.fetch_memory(clnum, address, ln).values()))
    ...
    def read_strace_file(self):
        try:
            strace_file = open(qira_config.TRACE_FILE_BASE +
                               str(int(self.forknum)) + "_strace").read()
        except Exception as err:
            return "no strace"

        strace_file = ''.join(filter(lambda x: ord(x) < 0x80, strace_file))
        ret = []
        files = {}
        for f_file in strace_file.split("\n"):
            if f_file == '':
                continue
            f_file = f_file.split(" ")
            try:
                ...
                if fxn == "open" and return_code != -1:
                    ...
                    if mapp not in self.mapped:
                        ...
                        try:
                            try:
                                ld_file = open(
                                    os.environ['QEMU_LD_PREFIX'] + "/" + files[fil])
                            except Exception as err:
                                ld_file = open(files[fil])
                            ...
                            map_st = "*** mapping %s %s sz:0x%x off:0x%x @ 0x%X" % (
                                sha1(alldat).hexdigest(), files[fil], sz, off, return_code)
                            print map_st,
                            dat = alldat[off:off + sz]

                            self.program.static.add_memory_chunk(
                                return_code, dat)
                            print "done"
                        except Exception as err:
                            print err
    ...
    def analysis_thread(self):
        ...
        while True:
            ...
            if (self.maxclnum is None) or (self.db.get_maxclnum() != self.maxclnum):
                self.analysisready = False
    ...
    def load_base_memory(self):
        def get_forkbase_from_log(n_log):
            ret = struct.unpack(
                "i",
                open(
                    qira_config.TRACE_FILE_BASE +
                    str(n_log)).read(0x18)[0x10:0x14])[0]
            if ret == -1:
                return n_log
            else:
        ...
        # by Pin at this time, and would require explicit mach-o parsing and
        # stuff.)
        img_map = {}
        images_dir = qira_config.TRACE_FILE_BASE + \
            str(self.forknum) + "_images"
                ...
                for image in os.listdir(images_dir):
                    if os.path.isfile(images_dir + "/" + image):
                        ...
                        for offset in os.listdir(images_dir + "/" + image):
                            off_map[fhex(offset)] = images_dir + \
                                "/" + image + "/" + offset
                        img_map[unquote(image)] = off_map
            except Exception as err:
                print "Exception while dealing with _images/:", err

        for ln in base_file.read().split("\n"):
            ln = ln.split(" ")
            if len(ln) < 3:
                continue
            (ss, se) = ln[0].split("-")
            ...
            try:
                if fn in img_map:
                    off = max(
                        i for i in img_map[fn].iter_keys() if i <= offset)
                    with open(img_map[fn][off]) as image_map_file:
                        image_map_file.seek(offset - off)
                        dat = image_map_file.read(se - ss)
                else:
                    with open(fn) as image_map_file:
                        image_map_file.seek(offset)
                        dat = image_map_file.read(se - ss)
            except Exception as err:
                print "Failed to get", fn, "offset", offset, ":", err
                continue
