...
def draw_multigraph(dm_blocks):
    ...
    traces = []
    cls = []
    for i, block in enumerate(dm_blocks):
        h = ghex(block['start']) + "-" + ghex(block['end']) + \
            "\n" + block['dis'].replace("\n", "\\l") + "\\l"
        if h not in arr:
            arr.append(h)
        traces.append(arr.index(h))
        cls.append(block['clstart'])
  ...
  for i in range(0, len(traces)-1):
    #e = pydot.Edge(nodes[traces[i]], nodes[traces[i+1]], label=str(cls[i+1]), headport="n", tailport="s")
    te = [nodes[traces[i]], nodes[traces[i+1]]]
    if te not in edges:
      edges.append(te)
      cnts.append(1)
    else:
...
def get_blocks(gb_flow, static=True):
    ...
    for (address, length, clnum, ins) in gb_flow:
        dins[address] = ins
        if next_instruction is not None and next_instruction != address:
    ...
    for (address, length, clnum, ins) in gb_flow:
        ...
        if address in basic_block_starts:
            gb_blocks.append({'clstart': cchange[0],
                              'clend': last[0],
                              'start': cchange[1],
                              'end': last[1],
                              'dis': disasm(cchange[1], last[1])})
...
def do_function_analysis(dfa_flow):
    next_instruction = None
    ...
    for (address, length, clnum, ins) in dfa_flow:
        ...
        elif next_instruction is not None and next_instruction != address:
...
def do_loop_analysis(dla_blocks):
    #dla_blocks = dla_blocks[0:0x30]
    arr = []
    bb = []
    ab = []
    ...
    idx = 0
    for i, block in enumerate(dla_blocks):
        h = hex(block['start']) + "-" + hex(block['end'])
        if h not in arr:
            realblocks.append({'start': block['start'],
                               'end': block['end'],
                               'idx': idx})
            idx += 1
            arr.append(h)
    ...
    while did_update:
        ...
        for i in range(len(bb)):
            for j in range(1, i):
                ...
                if ((bb[i:i + j] == bb[i + j:i + j * 2]) and
                        (bb[i:i + j] == bb[i + j * 2:i + j * 3])):
                    loopcnt = 1
                    while bb[i + j * loopcnt:i + j * (loopcnt + 1)] == bb[i:i + j]:
                        loopcnt += 1
                    # print "loop",bb[i:i+j],"@",i,"with count",loopcnt
                    # document the loop
                    loop = {"clstart": dla_blocks[ab[i]]['clstart'],
                            "clendone": dla_blocks[ab[i + j - 1]]['clend'],
                            "clend": dla_blocks[ab[i + j * loopcnt]]['clend'],
                            "blockstart": ab[i],
                            "blockend": ab[i] + j - 1,
                            "count": loopcnt}
                    # remove the loop from the dla_blocks
...
def get_instruction_flow(gif_trace, program, minclnum, maxclnum):
    start = time.time()
    ret = []
    for i in range(minclnum, maxclnum):
        r = gif_trace.db.fetch_changes_by_clnum(i, 1)
        if len(r) != 1:
            continue
...
def guess_calling_conv(program, readregs, readstack):
    ...
    # convert read regs into strings
    readregs = map(lambda x: regs[x], readregs)
    ...
    for abi in filter(lambda x: x[0] != "_", static2.ABITYPE.__dict__):
        ...
        consistent = len(regs_cpy) <= len(static2.ABITYPE.__dict__[
            abi][0])  # we are consistent with this ABI
...
def analyse_calls(ac_trace):
    program = ac_trace.program
    for (addr, data, clnum, ins) in ac_trace.flow:
        instr = program.static[addr]['instruction']
        if not instr.is_call():
            continue

        endclnum = get_last_instr(ac_trace.dmap, clnum)
        if endclnum is None:
            continue

        # the function ran from start to ret... analyze what happened in there
        regs = ac_trace.db.fetch_registers(clnum)
        iptr = ac_trace.db.fetch_changes_by_clnum(clnum + 1, 1)[0]['address']
        ...
        if program.static['arch'] in ["i386", "x86-64", "arm"]:
            stack_reg = ["ESP", "RSP", "SP"][
                ["i386", "x86-64", "arm"].index(program.static['arch'])]
            ...
            for cl in xrange(clnum + 1, endclnum):
                changes = filter(
                    lambda x: x['type'] in "LS", ac_trace.db.fetch_changes_by_clnum(
                        cl, -1))
                argchanges = filter(lambda x: argrange[0] <= x[
                    'address'] <= argrange[1], changes)
                if len(argchanges) > 0:
                    seen = max(
                        max(map(lambda x: x['address'], argchanges)), seen)
                rchanges = filter(
                    lambda x: x['type'] in "RW", ac_trace.db.fetch_changes_by_clnum(
                        cl, -1))
                for rchange in rchanges:
                    regnum = rchange['address'] / rsize
                    if rchange['type'] is 'W' and regnum < nregs:
                        init_regs.add(regnum)
                        if ((regnum) in uninit_regs) and (
                                rchange['data'] == regs[regnum]):
            ...
            abi, nargs = guess_calling_conv(
                program, uninit_regs, ((seen - esp) / rsize) if (seen > 0) else 0)
            if func.abi is 'UNKNOWN':
                func.abi = abi
            func.nargs = max(nargs, func.nargs)


def display_call_args(instr, dca_trace, clnum):
    program = dca_trace.program
    regs = dca_trace.db.fetch_registers(clnum)
    iptr = dca_trace.db.fetch_changes_by_clnum(clnum + 1, 1)[0]['address']
    program.static.analyzer.make_function_at(program.static, iptr)
    ...
    endclnum = get_last_instr(dca_trace.dmap, clnum)
    ...
    if i < nargs:
        ...
        for j in xrange(i, nargs):
            ret += [
                ghex(
                    struct.unpack(
                        "<Q" if program.tregs[1] == 8 else "<I",
                        dca_trace.fetch_raw_memory(
                            clnum,
                            esp +
                            program.tregs[1],
                            program.tregs[1]))[0])]
    ...
    if endclnum:
        endregs = dca_trace.db.fetch_registers(endclnum)
...
def get_hacked_depth_map(ghdm_flow, program):
    start = time.time()
    return_stack = []
    ret = [0]
    last_clnum = None
    ...
    branch_delay = False
    for (address, length, clnum, ins) in ghdm_flow:
        # handing missing changes
        if last_clnum is not None and clnum != last_clnum + 1:
...
def get_vtimeline_picture(gvp_trace, minclnum, maxclnum):
    if gvp_trace.maxd == 0:
        return None
    ...
    for i in range(0, r, sampling):
        ...
        try:
            if gvp_trace.dmap[i] == -1:
                raise Exception("nope")
            c = int((gvp_trace.dmap[i] * 128.0) / gvp_trace.maxd)
            if i / sampling < im_y:
...
def analyze(a_trace, program):
    minclnum = a_trace.db.get_minclnum()
    maxclnum = a_trace.db.get_maxclnum()
    """
  if maxclnum > 10000:
  ...
  """
    ...
    flow = get_instruction_flow(a_trace, program, minclnum, maxclnum)
...
def slice(q_s_trace, inclnum):
    ...
    def get_stores(clnum):
        return set(map(lambda x: x['address'], filter(
            is_store, q_s_trace.db.fetch_changes_by_clnum(clnum, 100))))

    def get_loads(clnum):
        return set(map(lambda x: x['address'], filter(
            is_load, q_s_trace.db.fetch_changes_by_clnum(clnum, 100))))
    ...
    while clnum > max(0, inclnum - 100):
        ...
        if len(q_s_trace.db.fetch_changes_by_clnum(clnum, 100)) > 20:
            break
...
if __name__ == "__main__":
    ...
    flow = get_instruction_flow(
        trace,
        program,
        trace.db.get_minclnum(),
